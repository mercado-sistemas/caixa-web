...
const ESTOQUE_BFF = import.meta.env.VITE_ESTOQUE_BFF_URL || 'http://localhost:3001';
...
export default function App() {
  ...
  const carregarVendas = useCallback(async () => {
    try { setVendas(await api('/vendas')); } catch (e) { toast(e.message); }
  }, [toast]);

  useEffect(() => { if (sessao) carregar(); }, [sessao, carregar]);
  useEffect(() => { if (sessao && aba === 'vendas') carregarVendas(); }, [sessao, aba, carregarVendas]);
  ...
  async function buscarProduto(termo) {
    ...
    const r = await fetch(`${ESTOQUE_BFF}/api/produtos?busca=${encodeURIComponent(termo)}`,
  ...
  async function emitir() {
    try {
      ...
      toast(`<b>Venda ${v.num}</b> — R$ ${brl(v.total)}${v.troco ? ` · troco R$ ${brl(v.troco)}` : ''}`);
    } catch (e) { toast(e.message); }
  }
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: 16, display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <Botao primario={aba === 'caixa'} onClick={() => setAba('caixa')}>1-Caixa</Botao>
          <Botao primario={aba === 'vendas'} onClick={() => setAba('vendas')}>2-Vendas do dia</Botao>
        ...
        {aba === 'caixa' && (
          <div style={{ display: 'grid', gap: 14, gridTemplateColumns: 'minmax(0,1fr) 380px' }}>
            ...
            <div style={{ gridColumn: '1/-1', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <Botao onClick={async () => { if (pv) { await api(`/prevendas/${pv.num}`, { method: 'DELETE' }); setAtiva(null); carregar(); } }}>Cancela PV</Botao>
        ...
        {aba === 'vendas' && (
          <Janela titulo="Vendas do dia">
            ...
            <div style={{ overflow: 'auto', maxHeight: 480, border: '1px solid var(--linha)', borderRadius: 6 }}>
              <Tabela
                colunas={[
                  { chave: 'num', rotulo: 'Nº', num: true }, { chave: 'cliente', rotulo: 'Cliente' },
                  { chave: 'itens', rotulo: 'Itens', num: true }, { chave: 'total', rotulo: 'Total', num: true },
                  { chave: 'forma', rotulo: 'Pagamento' }, { chave: 'em', rotulo: 'Hora' },
                ]}
                dados={vendas}
                renderLinha={(v) => (
                  <tr key={v.num}>
                    ...
                    <td style={{ fontSize: 11 }}>{v.pagamentos?.map((p) => p.d).join(', ')}</td>
                    <td style={{ fontSize: 11 }}>{new Date(v.emitidaEm).toLocaleTimeString('pt-BR')}</td>
                  </tr>
                )}
              />
            </div>
            {vendas.length > 0 && (
              <div style={{ textAlign: 'right', fontWeight: 700, marginTop: 10, color: 'var(--azul)' }}>
                Total do dia: R$ {brl(vendas.reduce((s, v) => s + v.total, 0))} — {vendas.length} venda(s)
              </div>
            )}
          </Janela>
        )}
        ...
        {pagando && pv && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,33,61,.55)', display: 'grid', placeItems: 'center', padding: 14, zIndex: 100 }}
            ...
            <div style={{ width: '100%', maxWidth: 560 }}>
              <Janela titulo={`Pagamento — R$ ${brl(totalPV(pv))}`}>
